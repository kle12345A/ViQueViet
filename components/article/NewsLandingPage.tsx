    ...
    <div className="news-page">
      <section className="news-hero">
        <div className="container news-hero-grid">
          ...
          <figure className="news-hero-media"><Image src="/images/news/news-hero-factory.webp" alt="Nhân viên đang thao tác trong khu vực sản xuất" fill priority sizes="(max-width: 900px) 100vw, 58vw" /><figcaption>Tin từ Vị Quê Việt</figcaption></figure>
        </div>
      </section>
      ...
      <section className="news-content-section">
        <div className="container">
          ...
          {!isFiltering && featured && <article className="news-featured">
            <Link className="news-featured-image" href={`/tin-tuc/${featured.slug}`} tabIndex={-1} aria-hidden="true"><Image src={featured.image} alt="" fill sizes="(max-width: 900px) 100vw, 55vw" /></Link>
          ...
          {visible.length ? <div className="news-grid">{visible.map((article) => <NewsCard article={article} key={article.slug} />)}</div> : <div className="news-empty"><h2>Chưa tìm thấy bài viết phù hợp</h2><p>Hãy thử từ khóa khác hoặc chọn lại chủ đề.</p><button type="button" onClick={() => { setQuery(""); setCategory("Tất cả"); }}>Xem tất cả bài viết</button></div>}
...
function NewsCard({ article }: { article: NewsArticle }) {
  return <article className="news-card"><Link className="news-card-image" href={`/tin-tuc/${article.slug}`} tabIndex={-1} aria-hidden="true"><Image src={article.image} alt="" fill sizes="(max-width: 680px) 100vw, (max-width: 980px) 50vw, 33vw" /></Link><div className="news-card-body"><p className="news-eyebrow">{displayCategory(article.category)}</p><h3><Link href={`/tin-tuc/${article.slug}`}>{article.title}</Link></h3><p>{article.excerpt}</p><div><time dateTime={article.publishedAt}>{formatDate(article.publishedAt)}</time><Link href={`/tin-tuc/${article.slug}`}>Xem thêm <span>→</span></Link></div></div></article>;
}
